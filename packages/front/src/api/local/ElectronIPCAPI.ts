...
class ElectronIPCAPI implements IIPCAPI {
    ...
    events = {
        ...
        async onRequest(listener) {
            const [err, bindId] = await electron.events.onRequest(listener);
            if (err) throw new IPCError(err.message);

            return bindId;
        },
        async onDebug(listener) {
            const [err, bindId] = await electron.events.onDebug(listener);
            if (err) throw new IPCError(err.message);

            return bindId;
        },
        async off(bindId: number) {
            await electron.events.off(bindId);
        }
    } as const;
}
